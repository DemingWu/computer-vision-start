...
pcl::PointCloud<pcl::PointXYZI>::Ptr scanData_lidar(new pcl::PointCloud<pcl::PointXYZI>());
...
ros::Publisher* pubScanPointer_lidar = NULL;
...
void scanHandler(const sensor_msgs::PointCloud2::ConstPtr& scanIn)
{
  ...
  scanData_lidar->clear();
  pcl::fromROSMsg(*scanIn, *scanData_lidar);
  pcl::removeNaNFromPointCloud(*scanData_lidar, *scanData_lidar, scanInd);
 
  try{
    double row, pitch, yaw;
    listener->lookupTransform("map", "sensor", ros::Time(0), transform_point);
    tf::Matrix3x3(transform_point.getRotation()).getRPY(row, pitch, yaw);

    geometry_msgs::Quaternion geoQuat = tf::createQuaternionMsgFromRollPitchYaw(row, pitch, 0);
    transform_point.setRotation(tf::Quaternion(geoQuat.x, geoQuat.y, geoQuat.z, geoQuat.w));
    // tf::Vector3 vec_temp = transform_point.getOrigin();
    // transform_point.setOrigin(tf::Vector3(-vec_temp.x, -vec_temp.y, -vec_temp.z));
    transform_point.setOrigin(tf::Vector3(0, 0, 0));
  ...
  for (int i = 0; i < scanDataSize; i++)
  {
    ...
    scanData_lidar->points[i].x = pointX2;
    scanData_lidar->points[i].y = pointY2;
    scanData_lidar->points[i].z = pointZ2;
    ...
    // scanData_lidar->points[i].x = vec.x();
    // scanData_lidar->points[i].y = vec.y();
    // scanData_lidar->points[i].z = vec.z();
  }
  ...
  pcl::toROSMsg(*scanData_lidar, scanData3);
  scanData3.header.stamp = ros::Time().fromSec(odomRecTime);
  scanData3.header.frame_id = "vehicle";
  pubScanPointer_lidar->publish(scanData3);
}
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("lidar_twist", lidar_twist);
  ...
  ros::Publisher pubVehicleOdom = nh.advertise<nav_msgs::Odometry>("/state_estimation_vehicle", 5);
  ...
  ros::Publisher pubScan = nh.advertise<sensor_msgs::PointCloud2>("/registered_scan_vehicle", 2);
  pubScanPointer = &pubScan;

  ros::Publisher pubScan_lidar = nh.advertise<sensor_msgs::PointCloud2>("/lidar_data", 2);
  pubScanPointer_lidar = &pubScan_lidar;
